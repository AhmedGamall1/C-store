...
describe('DELETE /api/addresses/:id', () => {
  ...
  it('promotes a remaining address to default when the default is deleted', async () => {
    const { user, cookie } = await loggedInUser()
    const a = await createAddress(user.id, { isDefault: true, street: 'A' })
    const b = await createAddress(user.id, { isDefault: false, street: 'B' })
    ...
    const list = await request(app).get('/api/addresses').set('Cookie', cookie)
    // Deleting the default promotes exactly one of the remaining addresses.
    // Which one depends on id ordering, so we don't assert a specific id.
    const remaining = list.body.data
    expect(remaining).toHaveLength(2)
    const defaults = remaining.filter((x) => x.isDefault)
    expect(defaults).toHaveLength(1)
